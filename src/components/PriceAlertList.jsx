    ...
    return (
        <div style={{ marginTop: '1rem' }}>
            ...
            {(!alerts || alerts.length === 0) ? (
                ...
                <ul>{alerts.map(alert => (<li key={alert.id}>Alert for <strong>{alert.assetId}</strong> to <strong>{alert.direction.toLowerCase()}</strong> to <strong> ${alert.value.toLocaleString()}</strong></li>))}</ul>
            )}
        </div>
    );
}